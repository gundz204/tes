  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 mt-0 flex flex-col h-full">
        <h3 className="text-sm font-semibold text-gray-900 mb-1 min-h-20">{title}</h3>
        <p className="text-xs text-gray-500 mb-3 flex items-center gap-1">
          <FaUser className="text-[10px]" />
          {author}
        </p>
        <p className="text-sm text-gray-700 mb-4 leading-relaxed overflow-hidden line-clamp-2">
          {desc.length > 10 ? `${desc.slice(0, 50)}...` : desc}
        </p>
        <button className="inline-flex items-center bg-green-700 text-white text-sm px-4 py-2 rounded-md hover:bg-green-800 transition self-start">
          Selengkapnya
